...
export default function NotifikasiAdmin() {
  ...
  const handleItemClick = (item) => {
    handleRead(item.id);
    // Navigate ke halaman relevan berdasarkan tipe notifikasi
    if (item.tipe?.includes('submission') || item.tipe?.includes('delete_request')) {
      navigate('/admin/validasi');
    } else if (item.tipe?.includes('account')) {
      navigate('/admin/akun');
    }
  };
