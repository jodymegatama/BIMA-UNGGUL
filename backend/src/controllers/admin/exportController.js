...
export async function pdf(req,res){
  const buf = await exportService.buildPdfBuffer({ periodeId: req.query.periodeId, kelompok: req.query.kelompok });
...
export async function excel(req,res){
  const buf = await exportService.buildExcelBuffer({ periodeId: req.query.periodeId, kelompok: req.query.kelompok });
