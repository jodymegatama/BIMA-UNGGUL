  ...
  const [data, total] = await Promise.all([
    prisma.submissionItem.findMany({
      ...
      include: {
        ...
        createdBy: { select: { id: true, nip: true, name: true, email: true } },
  ...
  const [data, total] = await Promise.all([
    prisma.deleteRequest.findMany({
      ...
      include: {
        ...
        requestedBy: { select: { id: true, nip: true, name: true, email: true } },
        reviewedBy: { select: { id: true, nip: true, name: true, email: true } },
      },
      orderBy: { createdAt: 'desc' },
      skip,
      take: limit,
